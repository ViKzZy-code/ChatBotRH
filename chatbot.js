...
// Função para enviar mensagem simulando digitação
const sendMessageWithTyping = async (chat, to, text) => {
    await delay(2000);
...
client.on('message', async msg => {
    if (!msg.from.endsWith('@c.us')) {
        return; // Ignora mensagens de grupos ou outros tipos de chat
    }

    if (isSaudacao(msg.body)) {
